...
void fpCopy(FloatPoint *from, FloatPoint *to) {
  to->x = from->x;
  to->y = from->y;
  to->z = from->z;
}
...
void setXYZ(FloatPoint *fp) {
  //this function assumes that fpCopy was just called to set the current values of fp to current_units
...
void process_string(uint8_t  *instruction) {
  ...
  if (command_exists('G')) {
    ...
    case 1: //Coordinated Motion
      ...
      set_target(&fp);
      //Set Speed
      if (code == 1 && command_exists('F') && ((feedrate = getValue('F')) > 0)) feedrate_micros = calculate_feedrate_delay(feedrate);
    ...
    case 3://Counterclockwise arc
      ...
      for (s = 1; s <= steps; s++) {
        ...
        feedrate_micros = (feedrate > 0) ? calculate_feedrate_delay(feedrate) : getMaxSpeed();
